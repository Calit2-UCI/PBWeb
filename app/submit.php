...
function process_1a($db_connection, $patient_id, $day, $ampm, $start_time, $completion_time)
{
    // response1 is one long string of the 8 symptom questions
    // the commas separate the question groups
    // for example: 1234,2341,1231,1232,etc.
    $r1 = explode(",", $_POST['response1']);

    // create a new array to store the processed data
    $arr = array();

    // there are 8 question groups (pain, tired, etc.)
    for ($i = 0; $i < 8; ++$i) {
        $group = $r1[$i];

        // Some of the symptom question groups only have 2 or 3 questions, as opposed to 4.
        $num_questions = 4;
        if ($i > 4 && $i < 7) $num_questions = 3;
        if ($i == 7) $num_questions = 2;

        // Calculate average stuff
        $tot = 0;
        $zero_avg = $group[0] === 0; // If they answered no to first question (did you have X?) then avg is 0
        $missing_info = false;
        ...
        if ($zero_avg) {
            ...
            $arr[] = $tot / ($num_questions - 1);
        }
    }

    $response1_query = "INSERT INTO s1_msas_8_9 (patient_id, DayNum, ampm, start_time, completion_time,
    pain7, paint7, painf7, painb7, pain7t ,tired7, tiredt7, tiredf7, tiredb7, tired7t,
    sad7, sadt7, sadf7, sadb7, sad7t, itchy7, itchyt7, itchyf7, itchyb7, itchy7t,
    worry7, worryt7, worryf7, worryb7, worry7t, eat7, eatt7, eatb7, eat7t,
    vomit7, vomitt7, vomitb7, vomit7t, sleep7, sleepb7, sleep7t) VALUES
    (:patient_id, :DayNum, :ampm, :start_time, :completion_time, " . implode(",", $arr) . ")";
...
if (isset($_POST['patient_id']) && isset($_POST['day']) && isset($_POST['ampm']) && isset($_POST['patient_age'])) {
    ...
    if ($patient_age <= 9) {
        process_1a($db_connection, $patient_id, $day, $ampm, $start_time[0], $completion_time[0]);
    } else {
