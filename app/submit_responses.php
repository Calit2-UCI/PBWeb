...
if (isset($_POST['response_string']) && $_POST['response_string'] != null) {
    ...
    if (strlen($response_string) % 4 == 0) {
        ...
        $query_string = "INSERT INTO tg_test_responses (question_number, major, minor1, minor2, minor3) VALUES";
        foreach ($response_array as $response) {
            $num = $response['question_number'];
            $query_string .= " ('{$response['question_number']}', '{$response['major']}', '{$response['minor1']}', '{$response['minor2']}', '{$response['minor3']}') ,";
        }
