 ...
 */
  ...
  try {
    ...
    $query_response->bindValue(':bodtot', $response2['bodtot'], PDO::PARAM_INT);
   
    $query_response->bindValue(':wgr', $response2['wgr'], PDO::PARAM_INT);
   $query_response->bindValue(':wgrw', $response2['wgrw'], PDO::PARAM_INT);
    $query_response->bindValue(':wgrl', $response2['wgrl'], PDO::PARAM_INT);
...
$alert_array = explode(';', $response2['alerts']); 
...
for ($x = 0; $x < count($pids); $x++) {
  // echo "The number is:".$pids[$x]['patient_id'] .":::: \r\n";
